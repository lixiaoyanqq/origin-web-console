...
function x() {
if (V.offlineVirtualMachines && V.virtualMachines && V.pods) {
var e = _(V.pods).values().filter(function(e) {
return !!_.get(e, 'metadata.labels["kubevirt.io/vmUID"]');
}).keyBy('metadata.labels["kubevirt.io/vmUID"]').value(), t = _(V.virtualMachines).values().filter(function(e) {
return !!B(e);
}).keyBy(function(e) {
return B(e);
}).value(), n = [];
_.each(V.offlineVirtualMachines, function(r) {
...
}), V.monopods && (V.monopods = _(V.monopods).filter(function(e) {
...
var O = a.isHomePage;
V.catalogLandingPageEnabled = !d.DISABLE_SERVICE_CATALOG_LANDING_PAGE;
var U = t("annotation"), F = t("canI"), M = t("buildConfigForBuild"), q = t("deploymentIsInProgress"), z = t("imageObjectRef"), H = t("isJenkinsPipelineStrategy"), G = t("isNewerResource"), K = t("label"), W = t("podTemplate"), J = i.getPreferredVersion("buildconfigs"), Q = i.getPreferredVersion("builds"), Y = i.getPreferredVersion("appliedclusterresourcequotas"), Z = i.getPreferredVersion("daemonsets"), X = i.getPreferredVersion("deploymentconfigs"), ee = i.getPreferredVersion("deployments"), te = i.getPreferredVersion("horizontalpodautoscalers"), ne = i.getPreferredVersion("imagestreams"), re = i.getPreferredVersion("limitranges"), ae = i.getPreferredVersion("pods"), oe = i.getPreferredVersion("replicasets"), ie = i.getPreferredVersion("replicationcontrollers"), se = i.getPreferredVersion("resourcequotas"), ce = i.getPreferredVersion("routes"), le = i.getPreferredVersion("servicebindings"), ue = i.getPreferredVersion("clusterserviceclasses"), de = i.getPreferredVersion("serviceinstances"), me = i.getPreferredVersion("clusterserviceplans"), pe = i.getPreferredVersion("services"), ge = i.getPreferredVersion("statefulsets"), fe = i.getPreferredVersion("templates");
V.buildConfigsInstantiateVersion = i.getPreferredVersion("buildconfigs/instantiate");
var ve, he, ye = {}, be = {}, Se = {}, Ce = V.state = {
alerts: {},
builds: {},
clusterQuotas: {},
...
V.state.breakpoint = v.getBreakpoint();
var _e = _.throttle(function() {
var t = v.getBreakpoint();
V.state.breakpoint !== t && e.$evalAsync(function() {
V.state.breakpoint = t;
});
}, 50);
$(window).on("resize.overview", _e), V.showGetStarted = !1, V.showLoading = !0, V.filterByOptions = [ {
id: "name",
label: A.getString(N("Name"))
}, {
id: "label",
label: A.getString(N("Label"))
} ], V.filterBy = b.getLabelSelector().isEmpty() ? "name" : "label", V.viewByOptions = [ {
id: "app",
label: A.getString(N("Application"))
}, {
id: "resource",
label: A.getString(N("Resource Type"))
}, {
id: "pipeline",
label: A.getString(N("Pipeline"))
} ];
var we = function(e) {
...
return _.size(V.deploymentConfigs) + _.size(V.vanillaReplicationControllers) + _.size(V.deployments) + _.size(V.vanillaReplicaSets) + _.size(V.statefulSets) + _.size(V.daemonSets) + _.size(V.monopods) + _.size(V.state.serviceInstances) + _.size(V.mobileClients) + _.size(V.offlineVirtualMachines);
}, ke = function() {
return _.size(V.filteredDeploymentConfigs) + _.size(V.filteredReplicationControllers) + _.size(V.filteredDeployments) + _.size(V.filteredReplicaSets) + _.size(V.filteredStatefulSets) + _.size(V.filteredDaemonSets) + _.size(V.filteredMonopods) + _.size(V.filteredServiceInstances) + _.size(V.filteredMobileClients) + _.size(V.filteredOfflineVirtualMachines);
}, Ie = function() {
V.size = je(), V.filteredSize = ke();
var e = 0 === V.size, t = V.deploymentConfigs && V.replicationControllers && V.deployments && V.replicaSets && V.statefulSets && V.daemonSets && V.pods && V.state.serviceInstances;
Ce.expandAll = t && 1 === V.size, V.showGetStarted = t && e, V.showLoading = !t && e, V.everythingFiltered = !e && !V.filteredSize, V.hidePipelineOtherResources = "pipeline" === V.viewBy && (V.filterActive || _.isEmpty(V.pipelineBuildConfigs));
}, Re = function(e) {
...
if (V.routesToDisplayByApp = {}, V.routes) {
var e = [ V.filteredDeploymentConfigsByApp, V.filteredReplicationControllersByApp, V.filteredDeploymentsByApp, V.filteredReplicaSetsByApp, V.filteredStatefulSetsByApp, V.filteredDaemonSetsByApp, V.filteredMonopodsByApp ];
_.each(V.apps, function(t) {
var n = {};
_.each(e, function(e) {
var r = _.get(e, t, []);
_.each(r, function(e) {
var t = Pe(e), r = _.get(Ce, [ "servicesByObjectUID", t ], []);
_.each(r, function(e) {
var t = _.get(Ce, [ "routesByService", e.metadata.name ], []);
_.assign(n, _.keyBy(t, "metadata.name"));
});
});
}), V.routesToDisplayByApp[t] = Ee(n);
});
}
});
}, 300, {
...
V.filteredDeploymentConfigsByApp = Re(V.filteredDeploymentConfigs), V.filteredReplicationControllersByApp = Re(V.filteredReplicationControllers), V.filteredDeploymentsByApp = Re(V.filteredDeployments), V.filteredReplicaSetsByApp = Re(V.filteredReplicaSets), V.filteredStatefulSetsByApp = Re(V.filteredStatefulSets), V.filteredDaemonSetsByApp = Re(V.filteredDaemonSets), V.filteredMonopodsByApp = Re(V.filteredMonopods), V.apps = _.union(_.keys(V.filteredDeploymentConfigsByApp), _.keys(V.filteredReplicationControllersByApp), _.keys(V.filteredDeploymentsByApp), _.keys(V.filteredReplicaSetsByApp), _.keys(V.filteredStatefulSetsByApp), _.keys(V.filteredDaemonSetsByApp), _.keys(V.filteredMonopodsByApp)), s.sortAppNames(V.apps), Te();
}, Ae = function() {
var e = _.filter(V.deploymentConfigs, function(e) {
var t = we(e);
return _.isEmpty(Ce.pipelinesByDeploymentConfig[t]);
});
V.deploymentConfigsNoPipeline = _.sortBy(e, "metadata.name"), V.pipelineViewHasOtherResources = !(_.isEmpty(V.deploymentConfigsNoPipeline) && _.isEmpty(V.vanillaReplicationControllers) && _.isEmpty(V.deployments) && _.isEmpty(V.vanillaReplicaSets) && _.isEmpty(V.statefulSets) && _.isEmpty(V.monopods));
}, De = function() {
V.disableFilter = "pipeline" === V.viewBy && _.isEmpty(V.pipelineBuildConfigs);
}, $e = function(e) {
return b.getLabelSelector().select(e);
}, Be = [ "metadata.name", "spec.externalClusterServiceClassName" ], xe = function(e) {
return y.filterForKeywords(e, Be, Ce.filterKeywords);
}, Ve = function(e) {
switch (V.filterBy) {
case "label":
return $e(e);
...
case "name":
return !_.isEmpty(Ce.filterKeywords);
}
}, Oe = function() {
V.filteredDeploymentConfigs = Ve(V.deploymentConfigs), V.filteredReplicationControllers = Ve(V.vanillaReplicationControllers), V.filteredDeployments = Ve(V.deployments), V.filteredReplicaSets = Ve(V.vanillaReplicaSets), V.filteredStatefulSets = Ve(V.statefulSets), V.filteredDaemonSets = Ve(V.daemonSets), V.filteredMonopods = Ve(V.monopods), V.filteredPipelineBuildConfigs = Ve(V.pipelineBuildConfigs), V.filteredServiceInstances = Ve(Ce.orderedServiceInstances), V.filteredMobileClients = Ve(V.mobileClients), V.filteredOfflineVirtualMachines = Ve(V.offlineVirtualMachines), V.filterActive = Le(), Ne(), Ie();
}, Ue = a.project + "/overview/view-by";
V.viewBy = localStorage.getItem(Ue) || "app", e.$watch(function() {
return V.viewBy;
}, function(e) {
localStorage.setItem(Ue, e), De(), Be = "app" === V.viewBy ? [ "metadata.name", "metadata.labels.app" ] : [ "metadata.name" ], Oe(), "pipeline" === V.viewBy ? b.setLabelSuggestions(be) : b.setLabelSuggestions(ye);
}), d.DISABLE_OVERVIEW_METRICS || (C.isAvailable(!0).then(function(e) {
Ce.showMetrics = e;
}), e.$on("metrics-connection-failed", function(e, t) {
o.isAlertPermanentlyHidden("metrics-connection-failed") || Ce.alerts["metrics-connection-failed"] || (Ce.alerts["metrics-connection-failed"] = {
type: "warning",
message: A.getString(N("An error occurred getting metrics.")),
links: [ {
href: t.url,
label: A.getString(N("Open Metrics URL")),
target: "_blank"
}, {
href: "",
label: A.getString(N("Don't Show Me Again")),
onClick: function() {
...
var t = Pe(e);
return t ? Fe(e) ? [ e ] : _.get(V, [ "state", "podsByOwnerUID", t ], []) : [];
}, qe = function(e, t) {
var n = Pe(e);
Ce.notificationsByObjectUID[n] = t || {};
}, ze = function(e) {
var t = Pe(e);
return t ? _.get(Ce, [ "notificationsByObjectUID", t ], {}) : {};
}, He = function(e) {
if (Pe(e)) {
var t = Me(e), n = R.getPodAlerts(t, a.project);
qe(e, n);
}
}, Ge = function(e) {
_.each(e, He);
...
var t = we(e);
return t ? _.get(V, [ "replicationControllersByDeploymentConfig", t ]) : [];
};
V.getPreviousReplicationController = function(e) {
var t = We(e);
...
_.each(V.deploymentConfigs, Je);
}, Ye = function(e) {
var t = Pe(e);
return t ? _.get(V, [ "replicaSetsByDeploymentUID", t ]) : {};
}, Ze = function(e) {
var t = R.getPausedDeploymentAlerts(e), n = Ye(e);
_.each(n, function(e) {
var n = ze(e);
_.assign(t, n);
}), qe(e, t);
}, Xe = function() {
_.each(V.deployments, Ze);
}, et = function() {
Ge(V.replicationControllers), Ge(V.replicaSets), Ge(V.statefulSets), Ge(V.daemonSets), Ge(V.monopods);
}, tt = _.debounce(function() {
e.$evalAsync(function() {
et(), Qe(), Xe();
});
}, 500), nt = function(e) {
_.isEmpty(e) || (b.addLabelSuggestionsFromResources(e, ye), "pipeline" !== V.viewBy && b.setLabelSuggestions(ye));
}, rt = function(e) {
_.isEmpty(e) || (b.addLabelSuggestionsFromResources(e, be), "pipeline" === V.viewBy && b.setLabelSuggestions(be));
}, at = function(e) {
return "Succeeded" !== e.status.phase && "Failed" !== e.status.phase && (!K(e, "openshift.io/deployer-pod-for.name") && (!U(e, "openshift.io/build.name") && "slave" !== K(e, "jenkins")));
}, ot = function() {
Ce.podsByOwnerUID = j.groupByOwnerUID(V.pods), V.monopods = _.filter(Ce.podsByOwnerUID[""], at), x();
}, it = function(e) {
return !!_.get(e, "status.replicas") || (!U(e, "deploymentConfig") || q(e));
}, st = function(e) {
return U(e, "deploymentConfig");
}, ct = function() {
if (V.deploymentConfigs && V.replicationControllers) {
var e = [];
V.replicationControllersByDeploymentConfig = {}, V.currentByDeploymentConfig = {}, Se = {};
var t = {}, n = {};
_.each(V.replicationControllers, function(r) {
var a = st(r) || "";
(!a || !V.deploymentConfigs[a] && _.get(r, "status.replicas")) && e.push(r);
var o = Se[a];
o && !G(r, o) || (Se[a] = r);
var i;
"Complete" === U(r, "deploymentStatus") && ((i = t[a]) && !G(r, i) || (t[a] = r)), it(r) && _.set(n, [ a, r.metadata.name ], r);
}), _.each(t, function(e, t) {
_.set(n, [ t, e.metadata.name ], e);
}), _.each(n, function(e, t) {
var n = p.sortByDeploymentVersion(e, !0);
V.replicationControllersByDeploymentConfig[t] = n, V.currentByDeploymentConfig[t] = _.head(n);
}), V.vanillaReplicationControllers = _.sortBy(e, "metadata.name"), Qe();
}
}, lt = function(e, t) {
...
V.replicaSets && ve && (V.replicaSetsByDeploymentUID = P.groupByControllerUID(V.replicaSets), V.currentByDeploymentUID = {}, _.each(V.replicaSetsByDeploymentUID, function(e, t) {
if (t) {
var n = ve[t], r = _.filter(e, function(e) {
return lt(e, n);
}), a = p.sortByRevision(r);
V.replicaSetsByDeploymentUID[t] = a, V.currentByDeploymentUID[t] = _.head(a);
}
}), V.vanillaReplicaSets = _.sortBy(V.replicaSetsByDeploymentUID[""], "metadata.name"), Xe());
}, dt = {}, mt = function(e) {
e && Ce.allServices && _.each(e, function(e) {
var t = [], n = Pe(e), r = W(e);
_.each(dt, function(e, n) {
e.matches(r) && t.push(Ce.allServices[n]);
}), Ce.servicesByObjectUID[n] = _.sortBy(t, "metadata.name");
});
}, pt = function() {
if (Ce.allServices) {
dt = _.mapValues(Ce.allServices, function(e) {
return new LabelSelector(e.spec.selector);
});
var e = [ V.deploymentConfigs, V.vanillaReplicationControllers, V.deployments, V.vanillaReplicaSets, V.statefulSets, V.daemonSets, V.monopods ];
_.each(e, mt), Te();
}
}, gt = function() {
var e = E.groupByService(V.routes, !0);
Ce.routesByService = _.mapValues(e, E.sortRoutesByScore), Te();
}, ft = function() {
Ce.hpaByResource = f.groupHPAs(V.horizontalPodAutoscalers);
}, vt = function(e) {
var t = M(e), n = V.buildConfigs[t];
if (n) {
V.recentPipelinesByBuildConfig[t] = V.recentPipelinesByBuildConfig[t] || [], V.recentPipelinesByBuildConfig[t].push(e);
var r = l.usesDeploymentConfigs(n);
_.each(r, function(t) {
Ce.recentPipelinesByDeploymentConfig[t] = Ce.recentPipelinesByDeploymentConfig[t] || [], Ce.recentPipelinesByDeploymentConfig[t].push(e);
}), Ae();
}
}, ht = {}, yt = function() {
ht = l.groupBuildConfigsByOutputImage(V.buildConfigs);
}, bt = function(e) {
var t = Pe(e);
if (t) return _.get(Ce, [ "buildConfigsByObjectUID", t ], []);
}, St = function(e) {
var t = [], n = bt(e);
_.each(n, function(e) {
var n = _.get(Ce, [ "recentBuildsByBuildConfig", e.metadata.name ], []);
t = t.concat(n);
});
var r = we(e);
_.set(Ce, [ "recentBuildsByDeploymentConfig", r ], t);
}, Ct = function(e, t) {
var n = Pe(t);
n && _.set(Ce, [ "buildConfigsByObjectUID", n ], e);
}, _t = function() {
var e = [];
V.deploymentConfigsByPipeline = {}, Ce.pipelinesByDeploymentConfig = {}, _.each(V.buildConfigs, function(t) {
if (H(t)) {
e.push(t);
var n = l.usesDeploymentConfigs(t), r = we(t);
_.set(V, [ "deploymentConfigsByPipeline", r ], n), _.each(n, function(e) {
Ce.pipelinesByDeploymentConfig[e] = Ce.pipelinesByDeploymentConfig[e] || [], Ce.pipelinesByDeploymentConfig[e].push(t);
});
}
}), V.pipelineBuildConfigs = _.sortBy(e, "metadata.name"), Ae(), rt(V.pipelineBuildConfigs), De();
}, wt = function() {
Ce.buildConfigsByObjectUID = {}, _.each(V.deploymentConfigs, function(e) {
var t = [], n = _.get(e, "spec.triggers");
_.each(n, function(n) {
var r = _.get(n, "imageChangeParams.from");
if (r) {
var a = z(r, e.metadata.namespace), o = ht[a];
_.isEmpty(o) || (t = t.concat(o));
}
}), t = _.sortBy(t, "metadata.name"), Ct(t, e), St(e);
});
}, Pt = function() {
_t(), wt();
}, jt = function() {
_.each(V.deploymentConfigs, St);
}, kt = function() {
if (Ce.builds && V.buildConfigs) {
V.recentPipelinesByBuildConfig = {}, Ce.recentBuildsByBuildConfig = {}, Ce.recentPipelinesByDeploymentConfig = {};
var e = {};
_.each(l.interestingBuilds(Ce.builds), function(t) {
var n = M(t);
H(t) ? vt(t) : (e[n] = e[n] || [], e[n].push(t));
}), V.recentPipelinesByBuildConfig = _.mapValues(V.recentPipelinesByBuildConfig, function(e) {
return l.sortBuilds(e, !0);
}), Ce.recentPipelinesByDeploymentConfig = _.mapValues(Ce.recentPipelinesByDeploymentConfig, function(e) {
return l.sortBuilds(e, !0);
}), Ce.recentBuildsByBuildConfig = _.mapValues(e, function(e) {
return l.sortBuilds(e, !0);
}), jt();
}
}, It = function() {
R.setQuotaNotifications(Ce.quotas, Ce.clusterQuotas, a.project);
};
V.clearFilter = function() {
b.clear(), V.filterText = "";
}, e.$watch(function() {
return V.filterText;
}, _.debounce(function(t, n) {
t !== n && (Ce.filterKeywords = y.generateKeywords(t), e.$evalAsync(Oe));
}, 50, {
maxWait: 250
})), e.$watch(function() {
return V.filterBy;
}, function(e, t) {
e !== t && (V.clearFilter(), Oe());
}), e.browseCatalog = function() {
w.toProjectCatalog(e.projectName);
}, b.onActiveFiltersChanged(function() {
e.$evalAsync(Oe);
}), V.startBuild = l.startBuild;
var Rt = function() {
if (Ce.bindingsByApplicationUID = {}, Ce.applicationsByBinding = {}, Ce.deleteableBindingsByApplicationUID = {}, !_.isEmpty(Ce.bindings)) {
var e = [ V.deployments, V.deploymentConfigs, V.vanillaReplicationControllers, V.vanillaReplicaSets, V.statefulSets, V.daemonSets ];
if (!_.some(e, function(e) {
return !e;
})) {
var t = c.getPodPresetSelectorsForBindings(Ce.bindings);
_.each(e, function(e) {
_.each(e, function(e) {
var n = Pe(e), r = new LabelSelector(_.get(e, "spec.selector"));
Ce.bindingsByApplicationUID[n] = [], Ce.deleteableBindingsByApplicationUID[n] = [], _.each(t, function(t, a) {
t.covers(r) && (Ce.bindingsByApplicationUID[n].push(Ce.bindings[a]), _.get(Ce.bindings[a], "metadata.deletionTimestamp") || Ce.deleteableBindingsByApplicationUID[n].push(Ce.bindings[a]), Ce.applicationsByBinding[a] = Ce.applicationsByBinding[a] || [], Ce.applicationsByBinding[a].push(e));
});
});
}), V.bindingsByInstanceRef = _.reduce(V.bindingsByInstanceRef, function(e, t, n) {
return e[n] = _.sortBy(t, function(e) {
var t = _.get(Ce.applicationsByBinding, [ e.metadata.name ]);
...
Ce.bindableServiceInstances = c.filterBindableServiceInstances(Ce.serviceInstances, Ce.serviceClasses, Ce.servicePlans), Ce.orderedServiceInstances = c.sortServiceInstances(Ce.serviceInstances, Ce.serviceClasses);
}, Tt = [], Nt = O ? {
skipErrorNotFound: !0
} : {};
k.get(a.project, Nt).then(_.spread(function(t, r) {
Ce.project = e.project = t, Ce.context = e.context = r;
var a = function() {
V.pods && h.fetchReferencedImageStreamImages(V.pods, Ce.imagesByDockerReference, Ce.imageStreamImageRefByDockerReference, r);
}, o = function(e) {
V.daemonSets = e.by("metadata.name"), mt(V.daemonSetData), mt(V.monopods), Ge(V.daemonSets), nt(V.daemonSets), Rt(), Oe(), S.log("daemonsets", V.daemonSets);
}, i = !1, s = function() {
i || (Tt.push(m.watch(Z, r, o, {
poll: L,
pollInterval: 6e4
})), i = !0);
}, c = function(e) {
...
if (Tt.push(m.watch(ae, r, function(e, t) {
V.pods = e.by("metadata.name"), ot(), a(), tt(), mt(V.monopods), Ge(V.monopods), nt(V.monopods), Oe(), t && "ADDED" !== t || l(), S.log("pods (subscribe)", V.pods);
})), Tt.push(m.watch(ie, r, function(e) {
V.replicationControllers = e.by("metadata.name"), ct(), mt(V.vanillaReplicationControllers), mt(V.monopods), Ge(V.vanillaReplicationControllers), nt(V.vanillaReplicationControllers), Rt(), Oe(), S.log("replicationcontrollers (subscribe)", V.replicationControllers);
})), Tt.push(m.watch(X, r, function(e) {
V.deploymentConfigs = e.by("metadata.name"), ct(), mt(V.deploymentConfigs), mt(V.vanillaReplicationControllers), nt(V.deploymentConfigs), Xe(), Pt(), jt(), Rt(), Oe(), S.log("deploymentconfigs (subscribe)", V.deploymentConfigs);
})), Tt.push(m.watch(oe, r, function(e) {
V.replicaSets = e.by("metadata.name"), ut(), mt(V.vanillaReplicaSets), mt(V.monopods), Ge(V.vanillaReplicaSets), nt(V.vanillaReplicaSets), Rt(), Oe(), S.log("replicasets (subscribe)", V.replicaSets);
})), Tt.push(m.watch(ee, r, function(e) {
ve = e.by("metadata.uid"), V.deployments = _.sortBy(ve, "metadata.name"), ut(), mt(V.deployments), mt(V.vanillaReplicaSets), nt(V.deployments), Rt(), Oe(), S.log("deployments (subscribe)", V.deploymentsByUID);
})), Tt.push(m.watch(Q, r, function(e) {
Ce.builds = e.by("metadata.name"), kt(), S.log("builds (subscribe)", Ce.builds);
})), Tt.push(m.watch(ge, r, function(e) {
V.statefulSets = e.by("metadata.name"), mt(V.statefulSets), mt(V.monopods), Ge(V.statefulSets), nt(V.statefulSets), Rt(), Oe(), S.log("statefulsets (subscribe)", V.statefulSets);
}, {
poll: L,
pollInterval: 6e4
})), m.list(Z, r, function(e) {
o(e), _.isEmpty(V.daemonSets) || s();
}), Tt.push(m.watch(pe, r, function(e) {
Ce.allServices = e.by("metadata.name"), pt(), S.log("services (subscribe)", Ce.allServices);
}, {
poll: L,
pollInterval: 6e4
})), Tt.push(m.watch(ce, r, function(e) {
V.routes = e.by("metadata.name"), gt(), S.log("routes (subscribe)", V.routes);
}, {
poll: L,
pollInterval: 6e4
})), Tt.push(m.watch(J, r, function(e) {
V.buildConfigs = e.by("metadata.name"), yt(), Pt(), kt(), Oe(), S.log("buildconfigs (subscribe)", V.buildConfigs);
}, {
poll: L,
pollInterval: 6e4
})), Tt.push(m.watch(te, r, function(e) {
V.horizontalPodAutoscalers = e.by("metadata.name"), ft(), S.log("autoscalers (subscribe)", V.horizontalPodAutoscalers);
}, {
poll: L,
pollInterval: 6e4
})), Tt.push(m.watch(ne, r, function(e) {
he = e.by("metadata.name"), h.buildDockerRefMapForImageStreams(he, Ce.imageStreamImageRefByDockerReference), a(), S.log("imagestreams (subscribe)", he);
}, {
poll: L,
pollInterval: 6e4
})), Tt.push(m.watch(se, r, function(e) {
Ce.quotas = e.by("metadata.name"), It();
}, {
poll: !0,
pollInterval: 6e4
})), Tt.push(m.watch(Y, r, function(e) {
Ce.clusterQuotas = e.by("metadata.name"), It();
}, {
poll: !0,
pollInterval: 6e4
...
V.mobileClients = e.by("metadata.name"), Oe(), S.log("mobileclients (subscribe)", e);
}, {
poll: L,
pollInterval: 6e4
})), e.KUBEVIRT_ENABLED) {
Tt.push(m.watch(D.offlineVirtualMachine, r, function(e) {
V.offlineVirtualMachines = e.by("metadata.name"), x(), Oe();
}, {
poll: L,
pollInterval: 6e4
}));
Tt.push(m.watch(D.virtualMachine, r, function(e) {
V.virtualMachines = e.by("metadata.name"), x(), Oe();
}, {
poll: L,
pollInterval: 6e4
}));
...
var p, g, f = {}, v = {};
u.SERVICE_CATALOG_ENABLED && F(de, "watch") && (p = function(e) {
var t = T.getServiceClassNameForInstance(e);
if (!t) return n.when();
var r = _.get(Ce, [ "serviceClasses", t ]);
return r ? n.when(r) : (f[t] || (f[t] = m.get(ue, t, {}).then(function(e) {
return Ce.serviceClasses[t] = e, e;
}).finally(function() {
delete v[t];
})), f[t]);
}, g = function(e) {
...
if (!t) return n.when();
var r = _.get(Ce, [ "servicePlans", t ]);
return r ? n.when(r) : (v[t] || (v[t] = m.get(me, t, {}).then(function(e) {
return Ce.servicePlans[t] = e, e;
}).finally(function() {
delete v[t];
})), v[t]);
}, Tt.push(m.watch(de, r, function(e) {
Ce.serviceInstances = e.by("metadata.name");
var t = [];
_.each(Ce.serviceInstances, function(e) {
var n = R.getServiceInstanceAlerts(e);
qe(e, n), t.push(p(e)), t.push(g(e));
}), I.waitForAll(t).finally(function() {
Et(), Oe();
}), nt(Ce.serviceInstances);
}, {
poll: L,
pollInterval: 6e4
}))), u.SERVICE_CATALOG_ENABLED && F(le, "watch") && Tt.push(m.watch(le, r, function(e) {
Ce.bindings = e.by("metadata.name"), V.bindingsByInstanceRef = _.groupBy(Ce.bindings, "spec.instanceRef.name"), Rt();
}, {
poll: L,
pollInterval: 6e4
})), m.list(re, r, function(e) {
Ce.limitRanges = e.by("metadata.name");
});
...
V.samplePipelineURL = w.createFromTemplateURL(t, e.projectName);
...
} ]), angular.module("openshiftConsole", [ "ngAnimate", "ngCookies", "ngResource", "ngRoute", "ngSanitize", "kubernetesUI", "registryUI.images", "ui.bootstrap", "patternfly.charts", "patternfly.navigation", "patternfly.sort", "patternfly.notification", "openshiftConsoleTemplates", "ui.ace", "extension-registry", "as.sortable", "ui.select", "angular-inview", "angularMoment", "ab-base64", "i18n", "openshiftCommonServices", "openshiftCommonUI", "webCatalog", "gettext" ]).config([ "$routeProvider", "$uibModalProvider", "HomePagePreferenceServiceProvider", function(e, t, n) {
var r, a = {
...
} ]), angular.module("openshiftConsole").controller("MonitoringController", [ "$routeParams", "$location", "$scope", "$filter", "APIService", "BuildsService", "DataService", "ImageStreamResolver", "KeywordService", "Logger", "MetricsService", "Navigate", "PodsService", "ProjectsService", "$rootScope", "gettextCatalog", "gettext", function(e, t, n, r, a, o, i, s, c, l, u, d, m, p, g, f, v) {
...
var h = r("isIE")(), y = [];
n.kinds = [ {
kind: f.getString(v("All"))
}, {
kind: "Pods"
}, {
label: f.getString(v("Deployments")),
kind: "ReplicationControllers"
}, {
kind: f.getString(v("Builds"))
}, {
kind: f.getString(v("Stateful Sets"))
} ], n.kindSelector = {
...
var b = r("isNil");
n.filters = {
hideOlderResources: b(e.hideOlderResources) || "true" === e.hideOlderResources,
text: ""
};
var S, C, w, P;
...
var j = r("orderObjectsByDate"), k = [ "metadata.name" ], I = [], R = function() {
n.filteredPods = c.filterForKeywords(P, k, I), n.filteredReplicationControllers = c.filterForKeywords(C, k, I), n.filteredReplicaSets = c.filterForKeywords(w, k, I), n.filteredBuilds = c.filterForKeywords(S, k, I), n.filteredStatefulSets = c.filterForKeywords(_.values(n.statefulSets), k, I);
}, E = function(e) {
...
n.filteredStatefulSets = c.filterForKeywords(_.values(n.statefulSets), k, I);
}, D = function() {
P = _.filter(n.pods, function(e) {
return !n.filters.hideOlderResources || "Succeeded" !== e.status.phase && "Failed" !== e.status.phase;
}), n.filteredPods = c.filterForKeywords(P, k, I);
}, B = r("isIncompleteBuild"), x = r("buildConfigForBuild"), V = r("isRecentBuild"), L = function() {
moment().subtract(5, "m");
S = _.filter(n.builds, function(e) {
if (!n.filters.hideOlderResources) return !0;
if (B(e)) return !0;
var t = x(e);
return t ? n.latestBuildByConfig[t].metadata.name === e.metadata.name : V(e);
}), n.filteredBuilds = c.filterForKeywords(S, k, I);
}, O = r("deploymentStatus"), U = r("deploymentIsInProgress"), F = function() {
C = _.filter(n.replicationControllers, function(e) {
return !n.filters.hideOlderResources || (U(e) || "Active" === O(e));
}), n.filteredReplicationControllers = c.filterForKeywords(C, k, I);
}, M = function() {
w = _.filter(n.replicaSets, function(e) {
return !n.filters.hideOlderResources || _.get(e, "status.replicas");
}), n.filteredReplicaSets = c.filterForKeywords(w, k, I);
};
...
n.project = e, n.projectContext = r, y.push(i.watch("pods", r, function(e) {
n.podsByName = e.by("metadata.name"), n.pods = j(n.podsByName, !0), n.podsByOwnerUID = m.groupByOwnerUID(n.pods), n.podsLoaded = !0, _.each(n.pods, E), D(), l.log("pods", n.pods);
})), y.push(i.watch({
resource: "statefulsets",
group: "apps",
version: "v1beta1"
}, r, function(e) {
n.statefulSets = e.by("metadata.name"), n.statefulSetsLoaded = !0, A(), l.log("statefulSets", n.statefulSets);
}, {
poll: h,
pollInterval: 6e4
})), y.push(i.watch("replicationcontrollers", r, function(e) {
n.replicationControllers = j(e.by("metadata.name"), !0), n.replicationControllersLoaded = !0, _.each(n.replicationControllers, T), F(), l.log("replicationcontrollers", n.replicationControllers);
})), y.push(i.watch("builds", r, function(e) {
n.builds = j(e.by("metadata.name"), !0), n.latestBuildByConfig = o.latestBuildByConfig(n.builds), n.buildsLoaded = !0, _.each(n.builds, N), L(), l.log("builds", n.builds);
})), y.push(i.watch({
group: "extensions",
resource: "replicasets"
}, r, function(e) {
n.replicaSets = j(e.by("metadata.name"), !0), n.replicaSetsLoaded = !0, M(), l.log("replicasets", n.replicaSets);
}, {
poll: h,
pollInterval: 6e4
})), n.$on("$destroy", function() {
...
n.filterKeywords = I = c.generateKeywords(n.filters.text), n.$apply(R);
}, 50, {
maxWait: 250
...
title: t.imagestream
} ], n.emptyMessage = s("Loading..."), n.imageStreamsVersion = r.getPreferredVersion("imagestreams"), n.imageStreamsVersion = r.getPreferredVersion("imagestreams"), n.imageStreamsVersion = r.getPreferredVersion("imagestreams");
var l = [];
...
var w = !1, P = t("annotation"), j = t("humanizeKind")(d), k = t("hasDeployment"), I = r.getPreferredVersion("builds"), R = r.getPreferredVersion("imagestreams"), E = r.getPreferredVersion("horizontalpodautoscalers"), T = r.getPreferredVersion("limitranges"), N = r.getPreferredVersion("pods"), A = r.getPreferredVersion("replicasets"), D = r.getPreferredVersion("replicationcontrollers"), $ = r.getPreferredVersion("resourcequotas"), B = r.getPreferredVersion("appliedclusterresourcequotas");
switch (d) {
case "ReplicaSet":
...
case "ReplicationController":
e.resource = D, e.healthCheckURL = f.healthCheckURL(n.project, "ReplicationController", n.replicaSet);
}
var x = {};
...
var L = t("deploymentStatus"), O = function(t) {
...
V.push(i.watch(e.resource, u, function(t) {
var n, r = [];
...
e.deployment = t, e.healthCheckURL = f.healthCheckURL(n.project, "Deployment", t.metadata.name, "apps"), V.push(i.watchObject(e.deploymentsVersion, t.metadata.name, u, function(t, r) {
...
t && l.fetchReferencedImageStreamImages([ t ], e.imagesByDockerReference, x, u);
}
};
...
object: t
}), V.push(i.watchObject(e.resource, n.replicaSet, u, function(t, n) {
"DELETED" === n && (e.alerts.deleted = {
type: "warning",
message: "This " + j + " has been deleted."
}), e.replicaSet = t, O(t), S(), H(), e.deployment && F();
})), e.deploymentConfigName && b(), V.push(i.watch(N, u, function(t) {
var n = t.by("metadata.name");
...
}), V.push(i.watch(e.resource, u, function(n, r, a) {
...
})), V.push(i.watch(R, u, function(e) {
var t = e.by("metadata.name");
l.buildDockerRefMapForImageStreams(t, x), H(), m.log("imagestreams (subscribe)", t);
})), V.push(i.watch(I, u, function(t) {
e.builds = t.by("metadata.name"), m.log("builds (subscribe)", e.builds);
})), V.push(i.watch(E, u, function(e) {
...
V.push(i.watch($, u, function(t) {
e.quotas = t.by("metadata.name");
}, {
poll: !0,
pollInterval: 6e4
})), V.push(i.watch(B, u, function(t) {
...
E() ? T(t) : _.has(e.strategyData, t) || ("Custom" !== e.strategyData.type ? e.strategyData[t] = {} : e.strategyData[t] = {
image: "",
command: [],
environment: []
...
}), E() && delete e.strategyData[R(e.originalStrategy)], "Rolling" === e.strategyData.type) {
...
}), _.has(e, "strategyData.customParams.environment") && (e.strategyData.customParams.environment = h.compactEntries(e.strategyData.customParams.environment)), e.updatedDeploymentConfig.spec.template.spec.imagePullSecrets = _.filter(e.secrets.pullSecrets, "name"), e.updatedDeploymentConfig.spec.strategy = e.strategyData, e.updatedDeploymentConfig.spec.triggers = A(), D(), l.update(C, e.updatedDeploymentConfig.metadata.name, e.updatedDeploymentConfig, e.context).then(function() {
m.addNotification({
type: "success",
...
name: "app",
value: ""
}, T = t("orderByDisplayName"), N = t("getErrorDetails"), A = {}, D = function() {
...
e.$on("$destroy", D);
var $ = i.getPreferredVersion("configmaps"), B = i.getPreferredVersion("limitranges"), x = i.getPreferredVersion("imagestreams"), V = i.getPreferredVersion("imagestreamtags"), L = i.getPreferredVersion("secrets"), O = i.getPreferredVersion("resourcequotas"), U = i.getPreferredVersion("appliedclusterresourcequotas");
...
link: function(d) {
d.serviceAccountsVersion = n.getPreferredVersion("serviceaccounts"), d.nameValidation = c, d.secretReferenceValidation = {
pattern: /^[a-zA-Z0-9\-_]+$/,
minLength: 8,
description: i.getString(o("Secret reference key must consist of lower-case, upper-case letters, numbers, dash, and underscore."))
}, d.secretAuthTypeMap = {
generic: {
...
p();
var o = m(d.newSecret.data, d.newSecret.authType);
r.create(n.objectToResourceGroupVersion(o), null, o, d).then(function(e) {
...
message: d.getString(m(e.kind)) + " " + e.metadata.name + " " + d.getString(m("can't be created in project")) + " " + e.metadata.namespace + ". " + d.getString(m("Can't create resource in different projects."))
}, !1) : (f.error = {
...
function C() {
var e, n;
A(), "Template" === f.resourceKind && f.templateOptions.process && !f.errorOccurred ? f.isDialog ? f.$emit("fileImportedFromYAMLOrJSON", {
...
function P() {
var t;
_.isEmpty(f.createResources) ? (t = _.head(f.updateResources), i.update(a.objectToResourceGroupVersion(t), t.metadata.name, t, {
namespace: f.input.selectedProject.metadata.name
}).then(function() {
if (!f.isDialog) {
var e = R(t.kind);
...
})) : (t = _.head(f.createResources), i.create(a.objectToResourceGroupVersion(t), null, t, {
namespace: f.input.selectedProject.metadata.name
}).then(function() {
if (!f.isDialog) {
var e = R(t.kind);
...
A(), N = u.getSecurityAlerts(f.createResources, f.input.selectedProject.metadata.name);
var t = e.quotaAlerts || [];
...
namespace: f.input.selectedProject.metadata.name
}).then(D) : (f.updateTemplate = 1 === f.updateResources.length && "Template" === f.updateResources[0].kind, f.updateTemplate ? y() : b()));
});
}, function(e) {
...
A(), s.toProjectOverview(f.input.selectedProject.metadata.name);
};
var B = e("displayName");
f.$on("importFileFromYAMLOrJSON", f.create), f.$on("$destroy", A);
} ]
};
...
return L(e) ? d.get(P, e.metadata.name, {
namespace: e.metadata.namespace
}) : n.when(e);
};
...
}, x(), i.$on("$routeChangeSuccess", x), N.change(function() {
...
onClick: function() {
return t.permanentlyHideNotification("openshift/token-warning"), !0;
}
} ]
});
}), r.on("error", function() {
...
var g = e("humanizeKind"), f = g(c.apiObject.kind), v = g(t.kind), h = {
namespace: c.project.metadata.name
};
a.update(n.kindToResource(t.kind), t.metadata.name, t, h).then(function() {
i.addNotification({
type: "success",
message: "Successfully added " + f + " " + c.apiObject.metadata.name + " to " + v + " " + t.metadata.name + ".",
links: [ {
...
i.addNotification({
type: "error",
message: "An error occurred  adding " + f + " " + c.apiObject.metadata.name + " to " + v + " " + t.metadata.name + ". " + r(n)
});
}).finally(function() {
c.disableInputs = !1;
});
...
E[t.id] ? E[t.id].load(a) : ((n = x(e)).data = a, r(function() {
...
function C() {
$ || (L = 0, _.each(p.metrics, function(e) {
v(e), f(e);
}));
}
function w(e) {
if (!$) if (L++, p.noData) p.metricsError = {
...
var R, E = {}, T = {}, N = n("resources.limits.memory"), A = n("resources.limits.cpu"), D = 30, $ = !1;
...
var B = e("upperFirst"), x = function(e) {
...
}, D = document.createDocumentFragment(), B = _.debounce(function() {
l.appendChild(D), D = document.createDocumentFragment(), t.autoScrollActive && A(), t.showScrollLinks || C();
}, 100, {
maxWait: 300
}), x = function(e) {
var t = a.defer();
...
t.resolve();
}), N.stop()) : t.resolve(), e || (B.cancel(), l && (l.innerHTML = ""), D = document.createDocumentFragment()), t.promise;
}, V = function() {
x().then(function() {
t.$evalAsync(function() {
if (t.run) {
...
}), x(!0)), r(a), !t.largeLog && n >= e.tailLines && t.$evalAsync(function() {
t.largeLog = !0;
}));
}), N.onClose(function() {
...
x(), g.off("resize", T), g.off("scroll", P), u && $(u).off("scroll", P);
...
g.serviceSelection = {}, g.projectDisplayName = t("displayName")(g.project), g.podPresets = v("pod_presets"), g.parameterData = {}, g.steps = [ c, l, u ], g.hideBack = l.hidden, "ServiceInstance" === g.target.kind ? (g.bindType = "secret-only", g.appToBind = null, g.serviceToBind = g.target, g.podPresets && w()) : (g.bindType = "application", g.appToBind = g.target, P());
...
function d() {
var e = _.get(P, "template.metadata.annotations.iconClass", "fa fa-clone");
...
function p() {
P.steps || (P.steps = [ P.selectStep, P.infoStep, P.configStep, P.resultsStep ]);
}
function g() {
w && (w(), w = void 0);
}
function f() {
P.infoStep.selected = !1, P.selectStep.selected = !0, P.configStep.selected = !1, P.resultsStep.selected = !1, P.nextTitle = u.getString(l("Next")) + " >", g(), C();
}
function v() {
...
function y(e) {
P.filterConfig.appliedFilters = e, b();
}
function b() {
P.filteredItems = P.catalogItems, P.filterConfig.appliedFilters && P.filterConfig.appliedFilters.length > 0 && _.each(P.filterConfig.appliedFilters, function(e) {
P.filteredItems = h(e.value, P.filteredItems);
}), _.includes(P.filteredItems, P.selectedTemplate) || P.templateSelected(), S();
}
function S() {
P.filterConfig.resultsCount = P.filteredItems.length, P.totalCount <= 1 ? $(".filter-pf.filter-fields input").attr("disabled", "") : $(".filter-pf.filter-fields input").removeAttr("disabled");
}
function C() {
P.unfilteredProjects || s.list().then(function(e) {
P.unfilteredProjects = _.toArray(e.by("metadata.name"));
}, function() {
P.unfilteredProjects = [];
}).finally(function() {
I();
});
}
var w, P = this, j = t("imageForIconClass"), k = t("annotation");
P.selectStep = {
id: "projectTemplates",
label: u.getString(l("Selection")),
view: "views/directives/process-template-dialog/process-template-select.html",
hidden: !0 !== P.useProjectTemplate,
allowed: !0,
valid: !1,
allowClickNav: !0,
onShow: f
...
allowClickNav: !0,
onShow: function() {
P.infoStep.selected = !0, P.selectStep.selected = !1, P.configStep.selected = !1, P.resultsStep.selected = !1, P.nextTitle = u.getString(l("Next")) + " >", g();
}
}, P.selectStep = {
id: "projectTemplates",
label: "Selection",
...
allowClickNav: !0,
onShow: function() {
P.infoStep.selected = !1, P.selectStep.selected = !1, P.configStep.selected = !0, P.resultsStep.selected = !1, P.nextTitle = u.getString(l("Create")), P.resultsStep.allowed = P.configStep.valid, w = e.$watch("$ctrl.form.$valid", function(e) {
P.configStep.valid = e && !P.noProjectsCantCreate && P.selectedProject, P.resultsStep.allowed = e;
});
}
}, P.resultsStep = {
id: "results",
...
allowClickNav: !1,
onShow: function() {
P.infoStep.selected = !1, P.selectStep.selected = !1, P.configStep.selected = !1, P.resultsStep.selected = !0, P.nextTitle = u.getString(l("Close")), g(), P.wizardDone = !0;
}
}, P.$onInit = function() {
P.loginBaseUrl = a.openshiftAPIBaseUrl(), P.preSelectedProject = P.selectedProject = P.project, P.useProjectTemplate && (P.project && (P.templateProject = P.project, P.templateProjectChange()), C()), P.noProjectsCantCreate = !1, e.$on("no-projects-cannot-create", function() {
P.noProjectsCantCreate = !0;
}), P.noProjectsCantCreate = !1, e.$on("no-projects-cannot-create", function() {
P.noProjectsCantCreate = !0;
}), P.projectEmptyState = {
...
appliedFilters: [],
onFilterChange: y
}, n.project || (P.showProjectName = !0);
}, P.$onChanges = function(e) {
e.template && P.template && (p(), P.iconClass = d(), P.image = m(), P.docUrl = k(P.template, "template.openshift.io/documentation-url"), P.supportUrl = k(P.template, "template.openshift.io/support-url"), P.vendor = k(P.template, "template.openshift.io/provider-display-name")), e.useProjectTemplate && p();
}, e.$on("templateInstantiated", function(e, t) {
P.selectedProject = t.project, P.currentStep = P.resultsStep.label;
}), P.$onDestroy = function() {
g();
}, P.next = function(e) {
return e.stepId === P.configStep.id ? (v(), !1) : e.stepId !== P.resultsStep.id || (P.close(), !1);
}, P.close = function() {
var e = P.onDialogClosed();
_.isFunction(e) && e();
}, P.onProjectSelected = function(t) {
P.selectedProject = t, P.configStep.valid = e.$ctrl.form.$valid && P.selectedProject;
}, P.templateSelected = function(e) {
P.selectedTemplate = e, P.template = _.get(e, "resource"), P.selectStep.valid = !!e, P.iconClass = d(), P.image = m(), P.docUrl = k(P.template, "template.openshift.io/documentation-url"), P.supportUrl = k(P.template, "template.openshift.io/support-url"), P.vendor = k(P.template, "template.openshift.io/provider-display-name");
}, P.templateProjectChange = function() {
P.templateProjectName = _.get(P.templateProject, "metadata.name"), P.catalogItems = {}, P.templateSelected(), r.getProjectCatalogItems(P.templateProjectName, !1, !0).then(_.spread(function(e, t) {
P.catalogItems = e, P.totalCount = P.catalogItems.length, b(), t && i.addNotification({
type: "error",
message: t
});
}));
...
var I = function() {
var e = _.reject(P.unfilteredProjects, "metadata.deletionTimestamp"), n = _.sortBy(e, t("displayName"));
P.searchEnabled = !_.isEmpty(e), P.templateProjects = c.orderByMostRecentlyViewed(n);
};
} ],
controllerAs: "$ctrl",
bindings: {
...
d.deploymentConfigsInstantiateVersion = n.getPreferredVersion("deploymentconfigs/instantiate"), d.replicationControllersVersion = n.getPreferredVersion("replicationcontrollers"), d.serviceBindingsVersion = n.getPreferredVersion("servicebindings"), d.deploymentConfigsVersion = n.getPreferredVersion("deploymentconfigs"), d.deploymentConfigsInstantiateVersion = n.getPreferredVersion("deploymentconfigs/instantiate"), d.deploymentConfigsLogVersion = n.getPreferredVersion("deploymentconfigs/log"), d.podsVersion = n.getPreferredVersion("pods"), d.podsLogVersion = n.getPreferredVersion("pods/log");
var v = function(e) {
...
t && (n.app.name = E(), n.runsAsRoot = s.runsAsRoot(t), n.ports = o.parsePorts(t), n.volumes = s.getVolumes(t), n.createImageStream = !0);
...
})) : C.length ? (D(C), n.disableInputs = !1) : A();
};
n.create = function() {
...
} ]), angular.module("openshiftConsole").directive("keyValueEditor", [ "$routeParams", "$timeout", "$filter", "APIService", "gettextCatalog", "keyValueEditorConfig", "keyValueEditorUtils", function(e, t, n, r, a, o, i) {
var s = n("humanizeKind"), c = n("canI"), l = 1e3;
return {
restrict: "AE",
scope: {
keyMinlength: "@",
...
var s = c(t.secretsVersion, "get"), u = c(t.configMapVersion, "get");
angular.extend(t, {
namespace: e.project,
unique: o,
forms: {},
placeholder: i.newEntry(),
setFocusKeyClass: "key-value-editor-set-focus-key-" + o,
setFocusValClass: "key-value-editor-set-focus-value-" + o,
uniqueForKey: i.uniqueForKey,
uniqueForValue: i.uniqueForValue,
...
t.entries.splice(e, n), !t.entries.length && t.addRowLink && i.addEntry(t.entries), t.forms.keyValueEditor.$setDirty();
},
...
onAddRow: function() {
i.addEntry(t.entries), i.setFocusOn("." + t.setFocusKeyClass);
},
onAddRowWithSelectors: function() {
i.addEntryWithSelectors(t.entries), i.setFocusOn("." + t.setFocusKeyClass);
},
...
t.addRowLink = e || "Add row", t.entries && !t.entries.length && i.addEntry(t.entries);
}), t.$watch("entries", function(e) {
e && !e.length && i.addEntry(t.entries), _.each(t.entries, function(e) {
i.altTextForValueFrom(e, t.namespace), i.setEntryPerms(e, s, u);
}), i.findReferenceValueForEntries(e, t.valueFromSelectorOptions);
}), t.$watch("valueFromSelectorOptions", function() {
i.findReferenceValueForEntries(t.entries, t.valueFromSelectorOptions);
});
} ]
};
...
e.totalUnread = E(e.notifications).length, e.hasUnread = !!e.totalUnread, r.$emit("NotificationDrawerWrapper.onUnreadNotifications", e.totalUnread);
});
}, N = function(e) {
...
r.$evalAsync(function() {
b.notificationGroups = [ R(a.project, V(x(C, w))) ], T();
});
}, O = function() {
_.each(S, function(e) {
e();
...
C[a.project] = $(B(e.by("metadata.name"))), L();
}, q = function(e, t) {
...
z(a.project, M), H(a.project, q), j(a.project), L();
});
};
angular.extend(b, {
drawerHidden: !0,
...
}), D(), L(), r.$emit("NotificationDrawerWrapper.onMarkAllRead");
},
notificationGroups: [],
...
m.markRead(e.uid), m.markCleared(e.uid), n.notifications.splice(t, 1), A(e), L();
},
markRead: function(e) {
e.unread = !1, m.markRead(e.uid), L();
},
close: function() {
b.drawerHidden = !0;
},
...
} ]).filter("podWarnings", [ "isContainerLoopingFilter", "isContainerFailedFilter", "isContainerUnpreparedFilter", "isTerminatingFilter", "gettext", "gettextCatalog", function(e, t, n, r, a, o) {
return function(i) {
var s = [];
return "Unknown" === i.status.phase && s.push({
reason: "Unknown",
pod: i.metadata.name,
...
}), "Running" === i.status.phase && i.status.containerStatuses && _.each(i.status.containerStatuses, function(c) {
if (!c.state) return !1;
t(c) && (r(i) ? s.push({
severity: "error",
...
pod: i.metadata.name,
container: c.name,
message: o.getString("The container {{name}} did not stop cleanly when terminated (exit code {{exitCode}}).", {
name: c.name,
...
pod: i.metadata.name,
container: c.name,
message: o.getString("The container {{name}} failed (exit code {{exitCode}}).", {
name: c.name,
exitCode: c.state.terminated.exitCode
})
})), e(c) && s.push({
...
pod: i.metadata.name,
container: c.name,
message: o.getString(a("The container {{name}} is crashing frequently. It must wait before it will be restarted again."), {
name: c.name
})
}), n(c) && s.push({
severity: "warning",
...
pod: i.metadata.name,
container: c.name,
message: o.getString(a("The container {{name}} has been running for more than five minutes and has not passed its readiness check."), {
name: c.name
})
});
}), s.length > 0 ? s : null;
