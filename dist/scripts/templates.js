angular.module('openshiftConsoleTemplates', []).run(['$templateCache', function($templateCache) {
  ...
  $templateCache.put('views/browse/config-maps.html',
    ...
    "</div>\n" +
    "<div ng-if=\"filterWithZeroResults\">\n" +
    "<h2><translate>The filter is hiding all config maps.</translate> <button type=\"button\" class=\"btn btn-link inline-btn-link\" ng-click=\"clearFilter()\">Clear All Filters</button></h2>\n" +
    "</div>\n" +
    "</div>\n" +
    "</div>\n" +
  ...
  $templateCache.put('views/browse/route.html',
    ...
    "<translate>Exposed on router</translate> '{{ingress.routerName}}' <span am-time-ago=\"admittedCondition.lastTransitionTime\"></span>\n" +
    "</span>\n" +
    "<span ng-if=\"admittedCondition.status === 'False'\">\n" +
    "<status-icon status=\"'Error'\"></status-icon>\n" +
    "<translate>Rejected by router</translate> '{{ingress.routerName}}' <span am-time-ago=\"admittedCondition.lastTransitionTime\"></span>\n" +
    "</span>\n" +
    "</div>\n" +
    ...
    "<dd translate>{{route.spec.tls.termination | humanizeTLSTermination}}</dd>\n" +
    "<dt translate>Insecure Traffic:</dt>\n" +
    "<dd translate>{{route.spec.tls.insecureEdgeTerminationPolicy || 'None'}}</dd>\n" +
  ...
  $templateCache.put('views/browse/routes.html',
    ...
    "</div>\n" +
    "<div ng-if=\"filterWithZeroResults\">\n" +
    "<h2><translate>The filter is hiding all routes.</translate> <button type=\"button\" class=\"btn btn-link inline-btn-link\" ng-click=\"clearFilter()\">Clear All Filters</button></h2>\n" +
    "</div>\n" +
    "</div>\n" +
    "</div>\n" +
  ...
  $templateCache.put('views/browse/service.html',
    ...
    "<div>\n" +
    "<traffic-table ports-by-route=\"portsByRoute\" routes=\"routesForService\" services=\"services\" show-node-ports=\"showNodePorts\" custom-name-header=\"Route\">\n" +
    "</traffic-table>\n" +
    "</div>\n" +
    "<p>\n" +
  ...
  $templateCache.put('views/browse/stateful-sets.html',
    ...
    "</div>\n" +
    "<div ng-if=\"filterWithZeroResults\">\n" +
    "<h2><translate>The filter is hiding all stateful sets.</translate> <button type=\"button\" class=\"btn btn-link inline-btn-link\" ng-click=\"clearFilter()\">Clear All Filters</button></h2>\n" +
    "</div>\n" +
    "</div>\n" +
    "</div>\n" +
  ...
  $templateCache.put('views/builds.html',
    ...
    "</div>\n" +
    "<div ng-if=\"filterWithZeroResults\">\n" +
    "<h2><translate>The filter is hiding all builds.</translate> <button type=\"button\" class=\"btn btn-link inline-btn-link\" ng-click=\"clearFilter()\">Clear All Filters</button></h2>\n" +
    "</div>\n" +
    "</div>\n" +
    "</div>\n" +
    ...
    "<span ng-if=\"latestBuild.status.reason && latestBuild.status.phase !== 'Cancelled'\" translate>{{latestBuild.status.reason | sentenceCase}}</span>\n" +
    "</div>\n" +
    "</td>\n" +
    "<td data-title=\"Duration\">\n" +
  ...
  $templateCache.put('views/create/fromimage.html',
    ...
    "</osc-form-section>\n" +
    "\n" +
    "<osc-form-section header=\"Build Configuration | translate\" about-title=\"Build Configuration | translate\" about=\"A build configuration describes how to build your deployable image.  This includes\n" +
    "                            your source, the base builder image, and when to launch new builds. | translate\" expand=\"true\" can-toggle=\"false\">\n" +
    "<div class=\"checkbox\">\n" +
    "<label>\n" +
  ...
  $templateCache.put('views/deployments.html',
    ...
    "</div>\n" +
    "<div ng-if=\"filterWithZeroResults\">\n" +
    "<h2><translate>The filter is hiding all deployments.</translate> <button type=\"button\" class=\"btn btn-link inline-btn-link\" ng-click=\"clearFilter()\">Clear All Filters</button></h2>\n" +
    "</div>\n" +
    "</div>\n" +
    "</div>\n" +
    "<div ng-if=\"!filterWithZeroResults\">\n" +
    ...
    "<span ng-switch-when=\"ImageChange\">\n" +
    "<span ng-if=\"cause.imageTrigger.from\">\n" +
    "<abbr title=\"{{cause.imageTrigger.from | imageObjectRef : null : true}}\" translate>Image change</abbr>\n" +
    "</span>\n" +
    "</span>\n" +
    "<span ng-switch-when=\"ConfigChange\" translate>Config change</span>\n" +
    "<span ng-switch-default>{{cause.type | translate}}</span>\n" +
  ...
  $templateCache.put('views/directives/add-config-to-application.html',
    ...
    "<legend><translate>Add this</translate> {{ctrl.apiObject.kind | humanizeKind | translate}} <translate>to application</translate>:</legend>\n" +
    ...
    "<ui-select-match placeholder=\"{{ctrl.applications.length ? 'Select an application' : 'There are no applications in this project' | translate}}\">\n" +
    "<span>\n" +
    "{{$select.selected.metadata.name}}\n" +
    "<small class=\"text-muted\">&ndash; {{$select.selected.kind | humanizeKind : true}}</small>\n" +
    "</span>\n" +
    ...
    "</span>\n" +
    "</div>\n" +
    "<legend><translate>Add</translate> {{ctrl.apiObject.kind | humanizeKind | translate}} <translate>as:</translate></legend>\n" +
    "<div class=\"radio\">\n" +
    "<label for=\"env\">\n" +
    ...
    "<input class=\"form-control\" name=\"envPrefix\" id=\"envPrefix\" placeholder=\"({{'optional'|translate}})\" type=\"text\" ng-pattern=\"/^[A-Za-z_][A-Za-z0-9_]*$/\" aria-describedby=\"env-prefix-help\" ng-disabled=\"ctrl.addType !== 'env' || !ctrl.application\" ng-model=\"ctrl.envPrefix\" autocorrect=\"off\" autocapitalize=\"off\" spellcheck=\"false\">\n" +
  ...
  $templateCache.put('views/directives/delete-link.html',
    "<a href=\"javascript:void(0)\" ng-click=\"openDeleteModal()\" role=\"button\" ng-attr-aria-disabled=\"{{disableDelete ? 'true' : undefined}}\" ng-class=\"{ 'disabled-link': disableDelete }\">{{label || ('删除')}}</a>"
  );
  ...
  $templateCache.put('views/directives/deploy-image.html',
    ...
    "<span class=\"pficon pficon-warning-triangle-o\" aria-hidden=\"true\"></span>\n" +
    "<translate>Service account</translate> <strong>default</strong> <translate>will need image pull authority to deploy images from</translate> <strong>{{istag.namespace}}</strong>. <translate>You can grant authority with the command</translate>:\n" +
    "<p>\n" +
    ...
    "</h2>\n" +
    "<ul>\n" +
    "<li ng-if=\"!import.namespace\"><translate>Image Stream</translate> <strong>{{app.name || \"&lt;name&gt;\"}}:{{import.tag || 'latest'}}</strong> <translate>will track this image.</translate></li>\n" +
    ...
    "<span class=\"help-block\">This name is already in use within the project. Please choose a different name.</span>\n" +
    "</div>\n" +
    "<osc-form-section header=\"{{'Environment Variables'|translate}}\" about-title=\"Environment Variables\" about=\"Environment variables are used to configure and pass information to running containers.\" expand=\"true\" can-toggle=\"false\" class=\"first-section\">\n" +
    "<key-value-editor entries=\"env\" key-placeholder=\"{{'Name'|translate}}\" key-validator=\"[A-Za-z_][A-Za-z0-9_]*\" key-validator-error=\"A valid environment variable name is an alphanumeric (a-z and 0-9) string beginning with a letter that may contain underscores.\" value-placeholder=\"{{'Value'|translate}}\" value-from-selector-options=\"input.selectedProject.metadata.uid && valueFromNamespace[input.selectedProject.metadata.name]\" add-row-link=\"{{'Add Value'|translate}}\" add-row-with-selectors-link=\"{{'Add Value from Config Map or Secret'|translate}}\"></key-value-editor>\n" +
    "</osc-form-section>\n" +
  ...
  $templateCache.put('views/directives/edit-config-map-or-secret.html',
    ...
    "</div>\n" +
    "<osc-file-input model=\"item.value\" drop-zone-id=\"drop-zone-{{$id}}\" help-text=\"{{'Enter a value for the' | translate}} {{type | translate}} {{'entry or use the contents of a file' | translate}}.\" read-as-binary-string=\"readAsBinaryString\" is-binary-file=\"isBinaryFile\"></osc-file-input>\n" +
    "<div ui-ace=\"{\n" +
    "          theme: 'eclipse',\n" +
    "          rendererOptions: {\n" +
    "            showPrintMargin: false\n" +
  ...
  $templateCache.put('views/directives/edit-environment-variables.html',
    ...
    "<h3><translate>Container</translate> {{container.name}}</h3>\n" +
  ...
  $templateCache.put('views/directives/label-editor.html',
    "<osc-form-section header=\"{{'Labels'|translate}}\" about-title=\"Labels\" about=\"{{'Labels are used to organize, group, or select objects and resources, such as pods.'|translate}}\" expand=\"expand\" can-toggle=\"canToggle\">\n" +
  ...
  $templateCache.put('views/directives/notifications/header.html',
    ...
    "<div class=\"col-xs-12\">\n" +
    "<em translate>{{notificationGroup.totalUnread}} Unread</em>\n" +
    "</div>\n" +
    "</div>\n" +
    "</div>"
  );
  ...
  $templateCache.put('views/directives/osc-form-section.html',
    ...
    "<span class=\"help action-inline\">\n" +
    "<i class=\"pficon pficon-help\"></i>\n" +
    "<a href data-toggle=\"tooltip\" data-original-title=\"{{about}}\"><translate>About</translate>{{aboutTitle|translate}}</a>\n" +
    "</span>\n" +
    "</li>\n" +
    "</ul>\n" +
    "</div>\n" +
  ...
  $templateCache.put('views/directives/process-template-dialog.html',
    ...
    "<pf-wizard title=\"{{!$ctrl.useProjectTemplate && ($ctrl.template | displayName) || 'Select from Project' | translate}}\" hide-sidebar=\"true\" step-class=\"order-service-wizard-step\" wizard-ready=\"$ctrl.wizardReady\" next-title=\"$ctrl.nextTitle\" next-callback=\"$ctrl.next\" on-finish=\"$ctrl.close()\" on-cancel=\"$ctrl.close()\" wizard-done=\"$ctrl.wizardDone\" current-step=\"$ctrl.currentStep\">\n" +
  ...
  $templateCache.put('views/edit/deployment-config.html',
    ...
    "<div class=\"form-group\">\n" +
    "<label translate>Environment Variables</label>\n" +
    "<key-value-editor entries=\"strategyData.customParams.environment\" key-validator=\"[a-zA-Z_][a-zA-Z0-9_]*\" key-validator-error-tooltip=\"A valid environment variable name is an alphanumeric (a-z and 0-9) string beginning with a letter that may contain underscores.\" value-from-selector-options=\"valueFromObjects\" add-row-link=\"{{'Add Value'|translate}}\" add-row-with-selectors-link=\"{{'Add Value from Config Map or Secret'|translate}}\"></key-value-editor>\n" +
    "</div>\n" +
    "</div>\n" +
    "<div ng-if=\"strategyData.type !== 'Custom'\">\n" +
    "<div class=\"form-group\">\n" +
  ...
  $templateCache.put('views/images.html',
    ...
    "</div>\n" +
    "<div ng-if=\"filterWithZeroResults\">\n" +
    "<h2><translate>The filter is hiding all image streams.</translate> <button type=\"button\" class=\"btn btn-link inline-btn-link\" ng-click=\"clearFilter()\">Clear All Filters</button></h2>\n" +
    "</div>\n" +
    "</div>\n" +
    "</div>\n" +
  ...
  $templateCache.put('views/modals/about-compute-units-modal.html',
    ...
    "<h3 translate>CPU</h3>\n" +
    "<p translate>\n" +
    "CPU通常是用单位<var>millicores(毫核)</var>来测量的。一个millicores(毫核)等于<sup>1</sup>&frasl;<sub>1000</sub>个CPU core(核)。\n" +
    "</p>\n" +
    "<pre translate>\n" +
  ...
  $templateCache.put('views/secrets.html',
    ...
    "</div>\n" +
    "<div ng-if=\"filterWithZeroResults\">\n" +
    "<h2><translate>The filter is hiding all secrets.</translate> <button type=\"button\" class=\"btn btn-link inline-btn-link\" ng-click=\"clearFilter()\">Clear All Filters</button></h2>\n" +
    "</div>\n" +
    "</div>\n" +
    "</div>\n" +
  ...
  $templateCache.put('views/service-instances.html',
    ...
    "</div>\n" +
    "<div ng-if=\"filterWithZeroResults\">\n" +
    "<h2><translate>The filter is hiding all provisioned services.</translate> <button type=\"button\" class=\"btn btn-link inline-btn-link\" ng-click=\"clearFilter()\">Clear All Filters</button></h2>\n" +
    "</div>\n" +
    "</div>\n" +
    "</div>\n" +
